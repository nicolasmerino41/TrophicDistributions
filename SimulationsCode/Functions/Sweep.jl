...
function degree_replicate_rows(simulation, metadata)
    ...
    for degree in DEGREE_CLASSES
        ...
        recursive = [row.mismatch for row in eligible]
        direct = [row.direct_mismatch for row in eligible]
        indirect = [row.indirect_increment for row in eligible]
        push!(rows, merge(metadata, (
            ...
            mean_mismatch=isempty(recursive) ? NaN : mean(recursive),
            mean_direct_mismatch=isempty(direct) ? NaN : mean(direct),
            mean_indirect_increment=isempty(indirect) ? NaN : mean(indirect),
            median_mismatch=isempty(recursive) ? NaN : median(recursive),
            mismatch_q90=mismatch_q90(recursive),
            mismatch_frac_gt=isempty(recursive) ? NaN : mean(recursive .> TAIL_THRESH),
...
function aggregate_degree_rows(replicate_rows)
    grouped = Dict{Tuple{Symbol,Int,Float64,Int},Vector{NamedTuple}}()
    for row in replicate_rows
        key = (row.environment, row.regime_id, row.target_r, row.degree)
        push!(get!(grouped, key, NamedTuple[]), row)
    end

    summary = NamedTuple[]
    for (key, rows) in grouped
        environment, regime_id, target_r, degree = key
        recursive = filter(isfinite, [row.mean_mismatch for row in rows])
        direct = filter(isfinite, [row.mean_direct_mismatch for row in rows])
        indirect = filter(isfinite, [row.mean_indirect_increment for row in rows])
        ...
        push!(summary, (
            ...
            n_communities_with_data=length(recursive),
            ...
            mean_direct_mismatch=isempty(direct) ? NaN : mean(direct),
            mean_indirect_increment=isempty(indirect) ? NaN : mean(indirect),
            sd_replicate_mean=length(recursive) < 2 ? NaN : std(recursive),
...
function sweep_all(;
    environments=ENVKINDS,
    regime_indices=collect(eachindex(regimes)),
    correlations=collect(range(CORR_RANGE[1], CORR_RANGE[2], length=N_CORR)),
    replicates=NREP
)
    ...
    for (env_idx, environment) in enumerate(environments),
        regime_id in regime_indices,
        (corr_idx, target_r) in enumerate(correlations),
    ...
    println("Grid: $(NX)x$(NY), S=$(S), basal fraction=$(BASAL_FRAC), minimum patch=$(Emin_patch), connectivity filter=$(USE_CONNECTIVITY_FILTER)")
    println("Degree classes: $(join(DEGREE_CLASSES, ", "))")
    println("Sweep: environments=$(length(environments)) × regimes=$(length(regime_indices)) × correlations=$(length(correlations)) × replicates=$(replicates)")
    ...
    return (
        ...
        correlation_values=Float64.(correlations)
    )
end

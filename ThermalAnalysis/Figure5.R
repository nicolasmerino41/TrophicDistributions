...
script_dir <- if (length(script_arg)) {
  ...
  normalizePath(file.path(getwd(), "ThermalAnalysis"), mustWork = TRUE)
}
root_dir <- normalizePath(file.path(script_dir, ".."), mustWork = TRUE)

OUTROOT <- file.path(root_dir, "Outputs", "thermal_metrics")
TRAITS  <- c("ctmin", "ctmax", "lt50", "ltmax")
OUTPUT_FILE <- file.path(OUTROOT, "Figure5.png")
...
niche_tiles <- transform(niche_df, height = y, width = dx * 1.02)
...
make_node_plot <- function(trait, bg_color) {
  ...
  df <- read.csv(node_file, stringsAsFactors = FALSE, check.names = FALSE)
  
  if (trait == "ctmax" && all(c("pred_ctmax", "mean_prey_ctmax") %in% names(df))) {
    names(df)[names(df) == "pred_ctmax"] <- "pred_trait"
    names(df)[names(df) == "mean_prey_ctmax"] <- "mean_prey_trait"
  }
  
  df <- df[is.finite(df$pred_trait) & is.finite(df$mean_prey_trait), , drop = FALSE]
  ...
  ggplot(df,
    ...
    labs(
      x = expression(bold("Consumer thermal limit (" * degree * "C)")),
      y = expression(bold("Mean resource thermal limit (" * degree * "C)"))
    ) +
